...

{
  services.sxhkd = {
    ...
    keybindings = {
      ...
      "XF86Search" = "xrandr --output eDP-1 --brightness $(echo \"$(xrandr --verbose | grep -m 1 'Brightness:' | cut -f2 -d ' ') + 0.1\" | bc)";
      "XF86Explorer" = "xrandr --output eDP-1 --brightness $(echo \"$(xrandr --verbose | grep -m 1 'Brightness:' | cut -f2 -d ' ') - 0.1\" | bc)";
